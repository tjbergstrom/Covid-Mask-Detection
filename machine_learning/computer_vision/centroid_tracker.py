...
#
# A centeroid_tracker is the location of a trackable object across frames.
# It has a list of objects including the centerpoint,
# and the actual box coordinates of the face that it tracks.
...
class Centroid_Tracker:
	...
	def register(self, centroid, box):
		self.objects[self.next_objectID] = (centroid, box)
	...
	def update(self, boxs):
		...
		for (i, (start_x, start_y, end_x, end_y)) in enumerate(boxs):
		...
		if len(self.objects) == 0:
			for i in range(0, len(input_centroids)):
				self.register(input_centroids[i], input_boxs[i])
		else:
			...
			for (row, col) in zip(rows, cols):
				...
				self.objects[objectID] = (input_centroids[col], input_boxs[col])
			...
			if D.shape[0] >= D.shape[1]:
				...
				for col in unused_cols:
					self.register(input_centroids[col], input_boxs[i])
		return self.objects
